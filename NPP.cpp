...
// Main function for NPP
std::tuple<std::vector<Process_p>, std::vector<Time_process>> NPP(std::string arrival, std::string burst, std::string priority) {
    ...
    std::vector<Process_p> processes;
    ...
    std::vector<std::pair<size_t, Process_p>> queue;
    ...
    */
    unsigned at_num, bt_num, pr_num, time_sum = 0, id_num = 1;
    ...
    std::sort(processes.begin(), processes.end(), [](const Process_p& a, const Process_p& b)
    ...
    while (time < time_sum) {
        ...
        if (!queue.empty()) {
            ...
            std::sort(queue.begin(), queue.end(), [](const std::pair<size_t, Process_p>& a, const std::pair<size_t, Process_p>& b)
